...
#' Costumized Wrapper to VAST from FishStatsUtils::fit_model 
#'
#' \code{fit_vast} fits a spatio-temporal model to data 
#'
...
fit_vast = function( settings, Lat_i, Lon_i, t_i, b_i, a_i, c_iz=rep(0,length(b_i)),
  ...
  # New stuff
  newregions = c("NorthSeaIBTS")
  if(settings$Region==newregions){
    if(settings$Region=="NorthSeaIBTS")
      ...
      extrapolation_list = nsibts_grid    
  } else {
  extrapolation_args_default = list(Region=settings$Region, strata.limits=settings$strata.limits, zone=settings$zone,
    max_cells=settings$max_cells)
  extrapolation_args_input = combine_lists( input=extra_args, default=extrapolation_args_default, args_to_use=formalArgs(make_extrapolation_info) )
  ...
  if(exists("check_fit") & test_fit==TRUE ){
    ...
    if( problem_found==TRUE ){
      message("\n")
      message("Please change model structure to avoid problems with parameter estimates and then re-try; see details in `?check_fit`\n", call.=FALSE)
